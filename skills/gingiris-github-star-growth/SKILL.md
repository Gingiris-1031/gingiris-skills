...
先检查 `Go / Fix / No-Go`：Quick Start 是否由陌生开发者验证、Demo 是否可复现、
License/安全/隐私是否清楚、Issues/PR/社区是否有 owner、是否跟踪 activation/D7/
contributor/qualified lead。No-Go 时停止冲量，先修产品与文档。

统一看板字段：`date | source | content URL | spend | repo views | stars | installs |
activated | D7 retained | contributors | qualified leads | evidence strength`。
2h 只排异常，24h 决定继续/改角度/暂停，72h 选第二波，7d 决定长期复用。
原生数据拿不到写 `数据不可用`。

禁止买 Star/账号、刷票、激励或协调虚假 Upvote、隐性付费合作、批量抓取并骚扰、
AI 冒充真人互动。平台规则、KOL 报价、账号数据与转化率必须带日期并在执行周复核。

## Star 地区分布 & 发布时序（诊断信号，不是 Trending 规则）

> 适用于 launch 阶段（和 `gingiris-opensource` 配合使用）

- GitHub 未公开“单一国家/地区占比不超过 20%”的 Trending 门槛；只把地域分布
  用于判断流量是否匹配目标市场。
- 工具：**star-history.com**（涨幅趋势）· **OSS Insight**（国家分布，国人开发）
- 发布时序按 ICP、支持覆盖和时区设计；“首周海外、次周国内”只是一种可测试方案。
...
- AFFiNE 的阶段分布属于特定历史案例，使用时标记为口述/项目证据，不外推为算法规则。
- 反模式：为了数字组织与目标用户无关的集中点 Star；它会扭曲市场验证，也可能触发平台风控。
...
- **关系型精准触达**：只联系公开讨论相关问题或同意接收产品信息的人，逐条个性化，
  遵守平台限制并提供退出方式；不要批量抓取竞品 follower 后群发。历史转化数字须标为口述证据。
